...
private:
   ...
   void test_destroyMain()
   {
      ...
      // Verify
      assertEquals(gps.isDead(), true);
      assertEquals(satellites.size(), 11);  // 3 pieces + 2 fragments
   }
   
   void test_destroyDistance0()
   {
      // Setup
      GPS gps;
      ...
      // Verify
      assertEquals(gps.isDead(), true);
      assertEquals(satellites.size(), 17);  // 3 pieces + 2 fragments
   }
   ...
   void test_destroyCenter()
   {
      ...
      // Verify
      assertEquals(center.isDead(), true);
      assertEquals(satellites.size(), 9);  // 3 pieces
   }
